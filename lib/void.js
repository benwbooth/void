...
function parse(data) {
  ...
  function toplevel() {
    ...
    this.indent = _indent;
    if (this.literate_comment.length) {
      console.log(arguments.callee.name, "add comment: ",render.cf(this.literate_comment));
      ret = ret.concat(this.literate_comment); 
      this.literate_comment = [];
    }
    console.log('exit', arguments.callee.name, [this.input], render.cf(ret));
    return ret;
  ...
  // state in this.indent.
  function indent() {
    console.log('enter', arguments.callee.name,[this.input]);
    this.indent = this.produce(spacesLine);
    console.log('exit', arguments.callee.name, [this.input], render.cf(this.indent));
    return this.indent;
  }
  function indent1() {
    console.log('enter', arguments.callee.name,[this.input]);
    this.indent = this.produce(spacesLine1);
    console.log('exit', arguments.callee.name, [this.input], render.cf(this.indent));
    return this.indent;
  }
  ...
  function literateComment() {
    if (this.indent.length === 0) {
      ...
      if (comment.length) {
        this.literate_comment = this.literate_comment.concat([{type: 'Comment', value: comment}]);
        console.log('literate comment: ',render.cf(this.literate_comment));
      }
    }
    return [];
  }
  ...
  function indentedList() {
    ...
    if (this.literate_comment.length) {
      literate_comment = literate_comment.concat(this.literate_comment);
      this.literate_comment = [];
    }

    var line = this.produce(indentedListLine);
    if (!line.length) {
      this.fail();
    }
    var line_comment = line.length && line[line.length-1].type === 'Comment'? 
        line.slice(-1) : [];
    console.log('line_comment=',line_comment);
    var head = line.length && line[line.length-1].type === 'Comment'? 
        line.slice(0,-1) : line;
    ...
    var rest = [];
    if (newline) {
      var parent_indent = this.indent;
      this.produce(indent);
      rest = this.produce(indentedListChildren(parent_indent));
    }
    var elements = head.concat(rest);

    var ret = elements.length <= 1? 
      elements: 
      [{type: 'List', value: elements}];

    if (literate_comment.length || line_comment.length) {
      ret[0].comment = literate_comment.concat(line_comment);
    }
  ...
  function indentedListChildren(parent_indent) {
    ...
    return function() {
      ...
      var ret = this.manyab(
        ...
        function() {
          var ret;
          if (this.indent.length <= parent_indent.length) {
            // dedent
            this.fail();
          }
          else if (this.indent.substring(0, parent_indent.length) === parent_indent) {
  ...
  function list(type, begin, end) {
    ...
    return function() {
      ...
      var first = this.produce(tokens);
      var rest = this.many(multilineList);
      this.indent = _indent;
      this.match(end);
      var ret = [{type: type, value: [].concat.apply([], first.concat(rest))}];
      if (ret[0].value.length && this.literate_comment.length) {
        ret = ret.concat(this.literate_comment);
        this.literate_comment = [];
      }
      console.log('exit', callee, [this.input], render.cf(ret));
  ...
  function multilineList() {
    ...
    var tokenline = this.produce(tokens);
    if (tokenline.length && this.literate_comment.length) {
      tokenline[0].comment = this.literate_comment; 
      this.literate_comment = [];
    }
    console.log('exit',arguments.callee.name, [this.input], render.cf(tokenline));
    return tokenline;
  }
  function tokens() {
    console.log('enter',arguments.callee.name,[this.input]);
  ...
  function string(delim) {
    ...
    return function () {
      console.log('enter', callee, [this.input]);
      var str = this.produce(quoted(delim));
      var ret = [{type: 'String', value: str}];
      console.log('exit', callee, [this.input], render.cf(ret));
      return ret;
    };
  }
  function quoted(delim) {
    var callee = arguments.callee.name;
    return function () {
      console.log('enter', callee, [this.input]);
    ...
    return function() {
      ...
      var ret = this.manyab(
          ...
          function() {
            ...
            var line = '';
            if (!newline.length) {
              if (!this.indent.length || this.indent.length < block_indent.length) {
                this.fail(); 
              }
              else {
                line = this.produce(/^[^\n]+\n/);
                if (!block_indent.length) {
                  block_indent = this.indent;
                }
                if (this.indent.substring(0, block_indent.length) !== block_indent) {
                  throw "Inconsistent whitespace usage in indented string literal";
                }
              }
            }
            var line_indent = block_indent.length && 
              this.indent.substring(0, block_indent.length) === block_indent?
                this.indent.substring(block_indent.length) : '';
            console.log('block_indent=',[block_indent],'line_indent=',[line_indent],'line=',[line]);

            return line_indent+line; 
          }).join('');
      console.log('indented string: ',ret);
  ...
  function operator() {
    console.log('enter',arguments.callee.name, [this.input]);
    var ret = this.choice(quoted('`'), bareOperator);
    console.log('exit',arguments.callee.name, [this.input], render.cf(ret));
    return ret;
  }
