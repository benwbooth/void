...
function parse(data) {
  ...
  function list() {
    ...
    var rest = this.option(function() {
        this.match(/^\n/); 
        return this.many(function() {return this.choice(literateComment, tokenLine)});
      }, []);
    this.match(/^\)/)
    var ret = {type: 'List', value: [].concat.apply([], first.concat(rest))};
    console.log('exit',arguments.callee.name, [this.input], render.cf(ret));
    return ret;
  }
  function squareBracketList() {
    ...
    var rest = this.option(function() {
      this.match(/^\n/); 
      return this.many(this.choice(literateComment, tokenLine))}, []);
    this.match(/^\]/); 
    var ret = {type: 'Vector', value: [].concat.apply([], first.concat(rest))};
    console.log('exit',arguments.callee.name, [this.input], render.cf(ret));
    return ret;
  }
  function curlyBraceList() {
    ...
    var rest = this.option(function() {
      ...
      return this.many(this.choice(literateComment, tokenLine))}, []);
    this.match(/^\}/); 
    var ret = {type: 'Environment', value: [].concat.apply([], first.concat(rest))};
    console.log('exit',arguments.callee.name, [this.input], render.cf(ret));
    return ret;
  }
  function tokenLine() {
  ...
  function tokens() {
    ...
    var ret = [].concat.apply([], elements).concat(postComment);
    console.log('exit',arguments.callee.name, [this.input], render.cf(ret));
    return ret;
  }
