...
function parse(data) {
  ...
  function literateComment() {
    ...
    if (this.indent.length === 0) {
      ...
      console.log('exit', arguments.callee.name, [this.input], render.cf([]));
      return []; 
    }
  }
  ...
  function indentedList() {
    ...
    this.produce(indent);
    var rest = this.produce(indentedListChildren(parent_indent));
