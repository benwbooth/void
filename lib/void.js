...
function parse(data) {
  ...
  function infixSuffix() {
    ...
    if (item_.length) {
      ...
      ret = [{type: 'SuffixOperator', value: op}];
    }
    console.log('exit',arguments.callee.name, [this.input], render.cf(ret));
    return ret;
  }
  ...
  function quoted(delim) {
    ...
    return function () {
      console.log('enter', callee, [this.input]);
      //var str = this.match(/^'((?:[^']|'')*)'/).replace((/''/g),"'");
      this.match(new RegExp('^'+delim));
      var ret = this.choice(maybeIndentedString(delim), quotedRest);
      console.log('exit', callee, [this.input], render.cf(ret));
      return ret;
    };
  }
  // Indented strings: started with /(?:'''|""")\s*\n/.
  // First (TODO: non-blank???) line determines the indentation level.
  // Provide a prefix macro `>` that removes the first line of a string
  // literal, to allow for multiline string literals with indentation.
  // Any lines consisting of only whitespace where indentation is 
  // inconsistent are converted into newlines ("\n").
  function maybeIndentedString(delim) {
    return function() {
      return this.choice();
    };
  }
