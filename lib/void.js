...
function parse(data) {
  ...
  function toplevel() {
    ...
    if (this.literate_comment.length) {
      ret = ret.concat(literate_comment); 
    }
    console.log('exit', arguments.callee.name, [this.input], render.cf(ret));
    return ret;
  }
  ...
  function literateComment() {
    if (this.indent.length === 0) {
      ...
      if (comment.length) {
        this.literate_comment = [{type: 'Comment', value: comment}];
      }
    }
    return [];
  }
  ...
  function indentedList() {
    ...
    if (this.literate_comment) {
      ret.comment = this.literate_comment;
      this.literate_comment = [];
    }
  ...
  function list(type, begin, end) {
    ...
    return function() {
      ...
      if (ret.value.length && this.literate_comment.length) {
        ret.comment = this.literate_comment;
        this.literate_comment = [];
      }
      console.log('exit', callee, [this.input], render.cf(ret));
