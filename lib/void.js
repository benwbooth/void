...
function parse(data) {
  ...
  function toplevel() {
    ...
    this.indent = '';
    var childrenList = this.many(indentedListChild(''));
  ...
  function literateComment() {
    ...
    var comment = this.chainl1(
        ...
        function() {
          this.match(/^\n/);
          return function(a,b) {
            return a+this.indent+b;
          };
        }).replace(/^\s*/,'').replace(/\s*$/,'');

    var ret = comment.length ? [{type: 'Comment', value: comment}] : [];
    console.log('exit', arguments.callee.name, [this.input], render.cf(ret));
    return ret;
  }
  ...
  function indentedList() {
    console.log('enter', arguments.callee.name, [this.input]);
    var head = this.produce(indentedListLine);
    var rest = this.many(indentedListChild(this.indent));
  ...
  function indentedListChild(parent_indent) {
    ...
    return function() {
      ...
      //attempt to parse a literate comment
      var comment = this.produce(literateComment);
      var ret;

      // indented list
      if (this.indent.length > parent_indent.length) {
        if (this.indent.substring(0, parent_indent.length) === parent_indent) {
          ret = comment.concat(this.produce(indentedList));
        }
        else {
  ...
  function elementCluster() {
    console.log('enter',arguments.callee.name,[this.input]);
    var ret = this.choice(comment, prefixInfix, firstItem); 
    console.log('exit',arguments.callee.name, [this.input], render.cf(ret));
    return ret;
  }
  ...
  function multilineList() {
    this.match(/^\n/); 
    var litcomment = this.produce(literateComment);
    var tokenline = this.produce(tokens);
    return litcomment.concat(tokenline);
  }
  function tokens() {
    console.log('enter',arguments.callee.name,[this.input]);
  ...
  function innerQuoted(delim, parsed) {
    ...
    return function() {
      console.log('enter', callee, [this.input]);
      var ret = (parsed+this.match(/^(?:[^']|'')*'/)).
        replace(new RegExp(delim+delim,'g'),delim);
      console.log('exit', callee, [this.input], render.cf(ret));
      return ret;
    };
  }
  // Indented strings: started with /(?:'''|""")\s*\n/.
  ...
  function indentedString(delim) {
    ...
    return function() {
      ...
      // instead.
      var d2 = this.option(new RegExp('^'+delim), '');
      if (!d2) return this.produce(innerQuoted(delim, ''));
      var d3 = this.option(new RegExp('^'+delim), '');
      if (!d3) return this.produce(innerQuoted(delim, d2));
      var ws = this.produce(spacesLine);
      var nl = this.option((/^\n/), '');
      if (!nl) return this.produce(innerQuoted(delim, d2+d3+ws));

      var block_indent='';
      var ret = this.chainl1(
          function() {
            return this.produce(indent1);
          }, 
          function() {
            // test if this is a blank line
            var blankline = this.option(/^\n/, '');
            var line = '';
            if (!blankline.length) {
            ...
            return function(a,b) {
              return a+
                (block_indent.length && 
                  b.substring(0, block_indent.length) === block_indent?
                  b.substring(block_indent.length) : '')+
                  line+"\n"; 
            };
          });
