...
function parse(data) {
  ...
  function Comment(data) { _.extend(this, data); }
  Comment.prototype = new AST();
  Comment.prototype.constructor = Comment;

  function Numeric(data) { _.extend(this, data); }
  Numeric.prototype = new AST();
  Numeric.prototype.constructor = Numeric;

  function PrefixOperator(data) { _.extend(this, data); }
  PrefixOperator.prototype = new AST();
  PrefixOperator.prototype.constructor = PrefixOperator;

  function InfixOperator(data) { _.extend(this, data); }
  InfixOperator.prototype = new AST();
  InfixOperator.prototype.constructor = InfixOperator;

  function PostfixOperator(data) { _.extend(this, data); }
  PostfixOperator.prototype = new AST();
  PostfixOperator.prototype.constructor = PostfixOperator;

  function Symbol(data) { _.extend(this, data); }
  Symbol.prototype = new AST();
  Symbol.prototype.constructor = Symbol;

  function StringValue(data) { _.extend(this, data); }
  StringValue.prototype = new AST();
  StringValue.prototype.constructor = StringValue;

  function List(data) { _.extend(this, data); }
  List.prototype = new AST();
  List.prototype.constructor = List;

  function Vector(data) { _.extend(this, data); }
  Vector.prototype = new AST();
  Vector.prototype.constructor = Vector;

  function document() {
    console.log('document', this.input);
    var doc = new List({values: this.many(toplevel)});
    this.eof();
    return doc;
  }
  ...
  function toplevel() {
    console.log('toplevel', this.input);
    var indent = this.produce(spacesLine);
    if (indent.length) {
      return this.produce(indentedList(indent));
    }
    else {
      return this.produce(literateComment);
    }
  }

  // Parse a series of literate comment lines
  function literateComment() {
    console.log('literateComment', this.input);
    return new Comment({comment: this.match(/^(?:\S[^\n]*\n)+/)});
  }
  ...
  function indentedListLine() {
    console.log('indentedListLine', this.input);
    var line = this.sepEndBy(elementCluster, spacesLine);
    this.match(/\n/);
    return line;
  }
  function elementCluster() {
    console.log('elementCluster', this.input);
    return this.choice(prefixInfix, firstItem); 
  }
  function prefixInfix() {
    console.log('prefixInfix', this.input);
    var op = this.produce(operator);
    var item_ = this.option(item, []);
    if (item_.length) {
      var more = this.many(infixPostfix);
      return [new PrefixOperator({symbol: op})] + item_ + [].concat.apply(null, more);
    }
    else {
      return [new InfixOperator({symbol: op})]; 
    }
  }
  function firstItem() {
    console.log('firstItem', this.input);
    var item_ = this.produce(item);
    var more = this.many(infixPostfix);
    return item_ + [].concat.apply(null, more);
  }
  function infixPostfix() {
    console.log('infixPostfix', this.input);
    var op = this.produce(operator);
    var item_ = this.option(item, []);
    if (item_.length) {
      return [new InfixOperator({symbol: op})] + item_;
    }
    else {
      return [new PostfixOperator({symbol: op})];
    }
  }

  // Parse an Item
  function item() {
    console.log('item', this.input);
  ...
  // Lists
  function list() {
    console.log('list', this.input);
    this.match(/^\(/); 
    var elements = this.many(this.choice(parseLiterateComment, tokenLine));
    this.match(/^\)/); 
    return new List({elements: [].concat.apply(null, elements)});
  }
  function squareBracketList() {
    console.log('squareBracketList', this.input);
    this.match(/^\[/); 
    var elements = this.many(this.choice(parseLiterateComment, tokenLine));
    this.match(/^\]/); 
    return new Vector({elements: [].concat.apply(null, elements)});
  }
  ...
  function curlyBraceList() {
    console.log('curlyBraceList', this.input);
    this.match(/^\{/); 
  ...
  function indentedString() {
    console.log('indentedString', this.input);
    this.match(/^(?:"""[ \t\r]*\n|'''[ \t\r]*\n)/);
    var indent = this.produce(spacesLine);
    return new StringValue({value: this.many(indentedStringLine(indent))});
  }
  ...
  function backslashString() {
    console.log('backslashString', this.input);
    return new StringValue({value: this.match(/^\\([^\s\\]\S*)/)}); 
  }

  // String literals
  function quoted() {
    console.log('quoted', this.input);
    return new StringValue({value: this.match(/^'([^']|'')*'/)});
  }
  function doubleQuoted() {
    console.log('doublequoted', this.input);
    return new StringValue({value: this.match(/^"([^"]|"")*"/)});
  }

  // Symbols
  function symbol() {
    console.log('symbol', this.input);
    return new Symbol({symbol: this.match(new RegExp('^[_'+categories.letter+'][_0-9'+categories.letter+']*'))}); 
  }

  // Operators
  function operator() {
    console.log('operator', this.input);
    return this.choice(backtickQuoted, bareOperator);
  }
  ...
  // One-line \\ Comments
  function comment() {
    console.log('comment', this.input);
    return new Comment({comment: this.match(/^\\\\([^\n]*\n)/)});
  }

  // Numeric literals
  function number() {
    console.log('number', this.input);
    return new Numeric({value: this.choice(zeroNum, decimal)});
  }
  function zeroNum() {
    this.match(/^(?=0)/); 
...
process.argv.slice(2).forEach(function(_) {
  console.log('"'+_+'"');
  console.log(parse(_));
});
